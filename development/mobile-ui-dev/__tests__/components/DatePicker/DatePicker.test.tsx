...
describe('DatePicker Component', () => {
  it('renders correctly with default props', () => {
    ...
    const { getByTestId } = render(<DatePicker onChange={onChangeMock} testID="test-datepicker" />);
  ...
  it('displays placeholder when no date is selected', () => {
    ...
    const { getByText } = render(<DatePicker onChange={onChangeMock} placeholder={placeholder} />);
  ...
  it('displays formatted date when a date is selected', () => {
    ...
    const { getByText } = render(<DatePicker value={testDate} onChange={onChangeMock} format="MM/DD/YYYY" />);
    expect(getByText('01/15/2023')).toBeTruthy();
  });

  it('opens calendar when input is pressed', () => {
    const onChangeMock = jest.fn();
    const { getByText, queryByText } = render(<DatePicker onChange={onChangeMock} placeholder="Select date" />);
  ...
  it('does not open calendar when disabled', () => {
    ...
    const { getByText, queryByText } = render(
      <DatePicker onChange={onChangeMock} placeholder="Select date" disabled />,
    );
  ...
  it('applies custom style', () => {
    ...
    const { getByTestId } = render(
      <DatePicker onChange={onChangeMock} style={{ backgroundColor: 'red' }} testID="test-datepicker" />,
    );
  ...
  it('formats date according to format prop', () => {
    ...
    const { getByText, rerender } = render(<DatePicker value={testDate} onChange={onChangeMock} format="MM/DD/YYYY" />);
    expect(getByText('01/15/2023')).toBeTruthy();

    // Test DD/MM/YYYY format
    rerender(<DatePicker value={testDate} onChange={onChangeMock} format="DD/MM/YYYY" />);
    expect(getByText('15/01/2023')).toBeTruthy();

    // Test YYYY-MM-DD format
    rerender(<DatePicker value={testDate} onChange={onChangeMock} format="YYYY-MM-DD" />);
